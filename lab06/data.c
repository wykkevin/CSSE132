 ...
 *
 * Edited by
 * NAMES:Yuankai Wang(Kevin), Mohammad Baqer
 *
 */
 ...
 */
void
dbe_print(struct db_entry* entry, FILE* f)
{
...
do_export_db(struct db_entry** db, const char* filename)
{
  ...
  // * close the file.
  FILE* write_f;
  write_f = fopen(filename, "w+");
  int i =0;
  int limit = db_count_entries(db);
  for(i=0;i<limit;i++){
	dbe_print(db[i],write_f);
	}
  fclose(write_f);
}
...
do_import_db(struct db_entry** db, const char* filename)
{
  ...
  // Don't forget to close the file that you open!
   FILE* read_f;
   read_f  = fopen(filename, "r");
   int temp = 0;
   char buf[128];
   char buf2[128];
   int count =0;
   while (fscanf(read_f, "%128[^=]=>%128[^\n]\n", buf, buf2)!=EOF){
...
getALine(char* dest, int maxlen, FILE* fd)
{
  ...
  // Before returning the string, you should replace newline character with a
  // null character
  fgets(dest, maxlen, fd);
  int output = strnlen(dest, maxlen);
  dest[output-1]='\0';
