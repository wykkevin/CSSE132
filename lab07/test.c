...
test_do_find_all_matches(void)
{
  ...
  do_find_all_matches(db,"Alice");
  TEST_ASSERT_EQUAL(strlen("Alice => A1\n"), strlen(G_PRINTF_OUTPUT));
  TEST_ASSERT_EQUAL_STRING("Alice => A1\n", G_PRINTF_OUTPUT);

  do_find_all_matches(db,"Bob");
  TEST_ASSERT_EQUAL(strlen("Bob => B1\n"), strlen(G_PRINTF_OUTPUT+12));
  TEST_ASSERT_EQUAL_STRING("Bob => B1\n", G_PRINTF_OUTPUT+12);

  do_find_all_matches(db,"Cindy");
  TEST_ASSERT_EQUAL(strlen("Cindy => C1\n"), strlen(G_PRINTF_OUTPUT+22));
  TEST_ASSERT_EQUAL_STRING("Cindy => C1\n", G_PRINTF_OUTPUT+22);

  do_find_all_matches(db,"Bo");
  TEST_ASSERT_EQUAL(strlen("Bob => B1\nBonnie => D1\n"), strlen(G_PRINTF_OUTPUT+34));
  TEST_ASSERT_EQUAL_STRING("Bob => B1\nBonnie => D1\n", G_PRINTF_OUTPUT+34);
